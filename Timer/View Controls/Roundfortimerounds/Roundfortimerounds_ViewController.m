//
//  Roundfortimerounds_ViewController.m
//  Timer
//
...
@interface Roundfortimerounds_ViewController ()

@end

@implementation Roundfortimerounds_ViewController

@synthesize time;
@synthesize second;
@synthesize round,reps;
...
- (void)viewDidLoad
{
    ...
    lbl_complet.text=[NSString stringWithFormat:@"%ld Left",(long)noround];
...
-(IBAction)StopTimer:(id)sender
{
    ...
    /* if (!contround==0)
    {
        ...
        round=[NSString stringWithFormat:@"%ld",[round integerValue]*contround];
   ...
   NSInteger  totaltime=[total_time integerValue] * (60 * 60);
    
    if (totaltime==0)
    {
        totaltime = [total_second integerValue] * 60;
    }
    else
    {
        totaltime = (totaltime)+ ([total_second integerValue] * 60);
        
        
    }
    
    
    totaltime = totaltime-remainingCounts;
    
    NSString *send=[NSString stringWithFormat:@"%ld",(long)totaltime];
    int set=[send intValue];
    NSString *strtime_left=[self formatTimeFromSeconds:set];
    
    NSInteger intround=[reps integerValue]*contround;
    
     reps=[NSString stringWithFormat:@"%ld",(long)intround];
    
    
    [ApplicationDelegate.dic_result setObject:strtime_left forKey:@"time"];
    
    [ApplicationDelegate.dic_result setObject:reps forKey:@"reps"];
