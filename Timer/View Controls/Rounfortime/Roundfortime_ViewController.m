...
@interface Roundfortime_ViewController ()

@end

@implementation Roundfortime_ViewController


- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.title=@"ROUNDS FOR TIME";
    ...
    txt_counter.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"00" attributes:@{NSForegroundColorAttributeName: color}];
...
-(IBAction)start
{
    ...
    else
    {
        ...
        else if(txt_counter.text.length==0)
        {
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:nil message:@"Please Enter Numbers of Round" delegate:self cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles: nil];
            [alert show];
            return ;
        }
        ...
        [ApplicationDelegate.dic_result setObject:@"ROUNDS FOR TIME" forKey:@"workout_name"];
        [ApplicationDelegate.dic_result setObject:lbl_time.text forKey:@"time"];
        
        [ApplicationDelegate.dic_result setObject:txt_counter.text forKey:@"rounds"];
        ...
        Roundfortimerounds_ViewController *timescreen = [work instantiateViewControllerWithIdentifier:@"Roundfortimerounds_ViewController"];
        
        timescreen.time = [yourWords firstObject];
        timescreen.second = [yourWords lastObject];
        
        timescreen.round=txt_counter.text;
        
        timescreen.countdown=lbl_counter.text;
        timescreen.reps=txt_round.text;
...
-(IBAction)settime:(id)sender
{
    ...
    [txt_counter resignFirstResponder];
    istimecap=true;
